...
export const processReading = async (sensorId, data) => {
    ...
    if (isViolation) {
        ...
        if (!existingAlert) {
            ...
            const recipients = Array.from(new Set([...assignedWorkerIds, ...managerIds]));
            if (recipients.length > 0) {
                await notificationService.sendPushNotification(recipients, `ТРИВОГА: ${sensor.name}`, `${violationType}: ${temperature}°C (Норма: ${min_temp}...${max_temp})`);
            }
            else {
                console.warn("⚠️ Alert created, but NO recipients found! (No assigned worker & no manager)");
            }
        }
        else {
            console.log(`⚠️ Alert continues for ${sensor.name}. Current: ${temperature}°C`);
        }
    }
    // Якщо різниця між Температурою і Точкою Роси менше 2 градусів -> Ризик!
    if (dewPoint !== undefined && temperature - dewPoint < 2.0) {
        console.warn(`💧 [WARNING] Condensation Risk at ${sensor.name}! T:${temperature}, DP:${dewPoint}`);
    }
};

...
export const getAllAlerts = async (storeId, status) => {
...
export const acknowledgeAlert = async (alertId, storeId, actor) => {
    ...
    // ЛОГУВАННЯ
    await auditService.logAction(storeId, actor.email, actor.id, "ACKNOWLEDGE_ALERT", `Alert ${alertId} acknowledged by user`);
    return updatedAlert;
};
// --- RESOLVE (Вирішити) ---
export const resolveAlert = async (alertId, storeId, actor) => {
    const alert = await prisma.alert.findFirst({
        where: { id: alertId, store_id: storeId },
    });
    if (!alert)
    ...
    // ЛОГУВАННЯ
    await auditService.logAction(storeId, actor.email, actor.id, "RESOLVE_ALERT", `Alert ${alertId} resolved by user`);
    return updatedAlert;
};

...
// Отримання даних
export const getDailyReportData = async (storeId) => {
...
// Генерація PDF
export const generateDailyReportPdf = async (storeId, res) => {
    ...
    res.setHeader("Content-Disposition", `attachment; filename*=UTF-8''${encodeURIComponent("Щоденний_Звіт.pdf")}`);
    doc.pipe(res);
    // Заголовок
    doc
        .font("Roboto")
    ...
    if (data.length === 0) {
        ...
        data.forEach((item, index) => {
            ...
            doc
                ...
                .font("Roboto")
                .fillColor("black")
                .text(`   Показники: T: ${item.temp}°C | H: ${item.hum}% | DP: ${item.dp}°C`);
